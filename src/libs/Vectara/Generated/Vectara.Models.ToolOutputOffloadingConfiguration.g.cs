...
namespace Vectara
{
    ...
    {
        /// <summary>
        /// Whether tool output offloading is active. Defaults to true. When disabled, tool outputs are always passed through to the agent verbatim.<br/>
        /// Example: true
        /// </summary>
        ...
        /// <summary>
        /// How a large tool output is handled after it is selected for offloading.<br/>
        /// In `artifact` mode, the full output is stored as a session artifact and replaced in the conversation with a compact reference containing the artifact id, size, shape, and hints for the available artifact tools. The agent can then use artifact_read, artifact_grep, or artifact_jq to retrieve just the parts it needs.<br/>
        /// In `truncate` mode, the output is shortened in place to fit within the target size. The head and tail are preserved and the middle is replaced with a short notice explaining that content was omitted. No artifact is created.<br/>
        /// When unset, the default is `artifact` if the agent has any of artifact_read, artifact_grep, or artifact_jq configured, and `truncate` otherwise.<br/>
        /// Example: artifact
        /// </summary>
        ...
        /// <summary>
        /// An absolute ceiling on single-output size. Any output above this many bytes is considered large enough to offload on its own, regardless of the model's context window. This prevents unusually large outputs from slipping through on models with very large context windows.<br/>
        /// Example: 1048576
        /// </summary>
        ...
        /// <summary>
        /// A hard floor on offloading. Tool outputs below this size are always passed through to the agent unchanged, even when cumulative context usage is high. This ensures that short, useful outputs are never unnecessarily truncated or replaced with a reference.<br/>
        /// Example: 4096
        /// </summary>
