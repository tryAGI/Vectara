...
namespace Vectara
{
    ...
    {
        /// <summary>
        /// Like `session_scoped`, but any session of the creating agent may resume the named session. The creating agent and target sub-agent enforce the scope.
        /// </summary>
        AgentScoped,
        /// <summary>
        ...
        Persistent,
        /// <summary>
        /// Resumes the session named by `session_key`, if provided; otherwise creates a new one. Only the parent session that originally created the named session may resume it. The parent session and target sub-agent enforce the scope, not the tool configuration.
        /// </summary>
        SessionScoped,
    }
