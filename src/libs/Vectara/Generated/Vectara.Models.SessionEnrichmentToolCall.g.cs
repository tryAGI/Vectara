...
{
    /// <summary>
    /// A single tool call run during session enrichment. It names one of the agent's enrichment-only tool configurations and supplies the arguments to invoke it with. The tool's own configuration, including its argument defaults and output_transform, lives on the tool configuration rather than on the call. Enrichment runs with no model to fill arguments in. The arguments must therefore include every required parameter of the called tool, either as a literal value or as a reference to an earlier call's output. A call that omits a required parameter is rejected when the agent is created or updated.
    /// </summary>
    public sealed partial class SessionEnrichmentToolCall
    {
        ...
        /// <summary>
        /// Dotted metadata path the tool's output is written to, after the tool's own configured output_transform is applied. Outputs are written to the session metadata in call order. A later write to a path replaces any value written earlier at that path. A call without a target path is compute only, so later calls can reference its output but nothing is written to the metadata.
        /// </summary>
