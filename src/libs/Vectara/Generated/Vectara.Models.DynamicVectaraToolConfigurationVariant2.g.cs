...
namespace Vectara
{
    ...
    ///
    /// </summary>
    public sealed partial class DynamicVectaraToolConfigurationVariant2
    {
        /// <summary>
        ...
        /// - session.caller.type / .id / .email - The platform-verified identity of the caller who created the session (`user`, `anonymous`, `api_key`, `app_client`, `agent`, or `service_account`. For `user` the `id` is the user's subject, which for a federated identity is the one asserted by the identity provider; for `anonymous` it is the `vis_` visitor id minted by the widget bootstrap; for `api_key` it is the SHA-256 hex digest of the key's secret value, stable per key but not the `aky_` api key id and never returned by `/v2/api_keys`; for `app_client` it is the app client's `client_id`; for `agent` it is the client id of the service account a Vectara-run agent acts as; for `service_account` it is the client id of a Vectara platform job's account, such as an evaluation run, a pipeline, or a simulation. `email` is present only for `user` callers. A session created by an agent reports `agent`). The platform derives it from the authenticated request. A client cannot set it or shadow it through session metadata. It resolves only during session-creation enrichment (`session_enrichment.tool_calls` and the `enrichment_only` tool configurations they name). A reference to it from a tool the agent calls mid-conversation stays unresolved and that call fails. An absent value (no email on the caller, or no authenticated identity) leaves the $ref unresolved and fails the call closed. In an `input_transform` jq expression the same absent value reads as `null` instead, so guard it there.<br/>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="DynamicVectaraToolConfigurationVariant2" /> class.
        /// </summary>
        /// <param name="type">
        /// This should always be `dynamic_vectara`.<br/>
        /// Default Value: dynamic_vectara<br/>
...
#endif
        public DynamicVectaraToolConfigurationVariant2(
            string type,
            string? toolId,
